...
if ! command -v docker >/dev/null 2>&1 || ! docker compose version >/dev/null 2>&1; then
  echo "Docker Compose is required to run benchmarks." >&2
  exit 1
fi
...
SECRET_KEY="benchmark-secret-key-with-at-least-32-bytes" \
S3_BUCKET="messenger-benchmark-uploads" \
S3_REGION="us-east-1" \
REDIS_URL="${TEST_REDIS_URL}" \
TEST_DATABASE_URL="${TEST_DATABASE_URL}" \
MESSAGE_RATE_LIMIT_PER_MINUTE=0 \
UPLOAD_RATE_LIMIT_PER_MINUTE=0 \
"${PYTHON_BIN}" -m benchmarks.suite "$@"
